...
EOF
    ...
    # Create the flow - CORRECTED to use bedrock-agent
    ...
    # Create flow alias - CORRECTED to use bedrock-agent
...
delete_resources() {
    ...
    # Delete flow alias first - CORRECTED to use bedrock-agent
    if [ ! -z "\$FLOW_ALIAS_ID" ] && [ ! -z "\$FLOW_ID" ]; then
        ...
        # Delete flow - CORRECTED to use bedrock-agent
    ...
    if [ ! -z "\$ROLE_NAME" ]; then
        aws iam detach-role-policy \
            --role-name \$ROLE_NAME \
            --policy-arn arn:aws:iam::\${ACCOUNT_ID}:policy/storybook-policy || true
...
show_info() {
    ...
    if [ -f flow_info.txt ]; then
        ...
        # Get flow details - CORRECTED to use bedrock-agent
...
EOF
    ...
    # Invoke the flow - CORRECTED to use bedrock-agent-runtime
    ...
    # Wait for flow to complete - May need to implement custom polling if wait command doesn't exist
    echo -e "\${YELLOW}Waiting for flow execution to complete...\${NC}"
    # Commenting out wait command as it may not exist - would need to implement polling
    # aws bedrock-agent-runtime wait flow-execution-complete \
    #    --flow-id \$FLOW_ID \
    ...
    while true; do
        ...
        echo -e "\${YELLOW}Flow execution status: \$STATUS. Waiting...\${NC}"
        sleep 30
    done
